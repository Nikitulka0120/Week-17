...
4. **Database Init (db-init)** - временный контейнер для инициализации таблиц и заполнения базы начальными данными перед стартом Backend.
5. **Frontend** (nginx + HTML/JS) - статический фронтенд с графиками Chart.js. Проксирует `/api/` запросы на Backend.
...
[Браузер] --HTTP--> [Frontend (nginx:3000)]
                  ...
                  [Backend (FastAPI:8000)] --SQLAlchemy--> [PostgreSQL:5432]
                    ...
                    gRPC (логи)                                   |
                         |                                (инициализация)
                         v                                        |
                  [Log Service (gRPC:50051)]                  [db-init]
```
