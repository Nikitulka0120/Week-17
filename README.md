...
Приложение отображает статистику товаров (просмотры и лайки). При запуске база данных PostgreSQL автоматически заполняется начальными товарами. Фронтенд показывает:
- Интерактивную фильтрацию товаров по категориям. При выборе категории перестраиваются и отображаются только графики и товары этой категории.
- 2 интерактивных графика (топ-7 по просмотрам и топ-7 по лайкам) с использованием Chart.js.
- Форму управления лайками, позволяющую ставить лайк или убирать его.
- Общую таблицу всех товаров.
...
| DELETE | `/api/products/{id}/like` | Убрать лайк товару |
